<template>
    <div class="max-w-[min(90%,1728px)] mx-auto grid justify-items-center mb-12">
        ...
        <div class="margin lg:gap-0 grid lg:grid-flow-col lg:grid-cols-[max-content,min(30%,_600px),min(35%,500px)] lg:justify-between gap-8 place-items-center" id="examples">
            <button class="p-[1em] text-2xl app-el w-max font-medium sm:text-3xl xl:text-4xl h-max transition-colors 2xl:text-5xl bg-black hover:bg-transparent" :style="`background-color: ${exampleColor.hex}; color: ${lightOrDark};`">click me</button>
...
watch(
    () => props.exampleColor,
    () => {
        console.log(props.exampleColor);
